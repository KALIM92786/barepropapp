# Run only Backend and Database services (skips Frontend)
...
Write-Host "Starting Backend and Database..."
# Start only the backend service. Docker Compose will start the database if 'depends_on' is configured.
...
# Run DB Setup inside backend container
Write-Host "Running database setup..."
docker compose run --rm backend npm run db:setup

# Restart backend to ensure connection and sync start
