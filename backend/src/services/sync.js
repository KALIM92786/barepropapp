...
const syncData = async () => {
    ...
    try {
        ...
        if (accountRes && accountRes.data) {
            ...
            await db.query(
                `INSERT INTO equity_snapshots (account_id, equity, balance, margin, free_margin) VALUES ($1, $2, $3, $4, $5)`,
                [ACCOUNT_ID, margin.equity, margin.balance, margin.margin, margin.free_margin]
            );
        ...
        if (dealsRes && dealsRes.data) {
            ...
            for (const deal of deals) {
                if (deal.status === 'closed') {
                    await db.query(
                        ...
                        [deal.id, ACCOUNT_ID, deal.ticker, deal.volume, deal.side, deal.open_price, deal.open_time, deal.close_price, deal.close_time, deal.profit, 'closed']
                    );
                }
            }
            ...
            for (const pos of openDeals) {
                ...
                await db.query(
                    `INSERT INTO positions (deal_id, account_id, ticker, volume, side, open_price, open_time, floating_profit)
                     VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`,
                    [pos.id, ACCOUNT_ID, pos.ticker, pos.volume, pos.side, pos.open_price, pos.open_time, pos.profit]
                );
            }
        ...
        if (ordersRes && ordersRes.data) {
            for (const order of ordersRes.data) {
                await db.query(
                    `INSERT INTO orders (id, account_id, ticker, volume, side, type, status, price, created_at)
                     VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
                     ON CONFLICT (id) DO UPDATE SET status = EXCLUDED.status`,
                    [order.id, ACCOUNT_ID, order.ticker, order.volume, order.side, order.type, order.status, order.price, Date.now()]
                );
            }
